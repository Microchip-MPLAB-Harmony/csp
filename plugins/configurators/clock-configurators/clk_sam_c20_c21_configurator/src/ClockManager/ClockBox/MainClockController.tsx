...
const MainClockController = (props: {
  ...
  const mainClockDiv = useKeyValueSetSymbol({ componentId, symbolId: 'CONF_CPU_CLOCK_DIVIDER' });
  let symbols: any = props.mainClockController
    .map((e) => e.symbol_id)
    .filter((e) => e !== undefined);
  symbols = removeDuplicates(symbols);

  const [dynamicSymbolsInfo] = useState(() => getDynamicSymbolsFromJSON(props.mainClockController));
  ...
  return (
    <div>
      ...
      />
      <PlainLabel
        disPlayText={mainClockDiv.selectedOption.replaceAll('DIV', '')}
