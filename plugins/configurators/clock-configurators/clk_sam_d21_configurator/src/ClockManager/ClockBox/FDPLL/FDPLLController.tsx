...
const FDPLLController = (props: {
  ...
  const DPLL_CLOCK_FREQ = useIntegerSymbol({ componentId, symbolId: 'DPLL_CLOCK_FREQ' });
  ...
  const FDPLLCustomFrequency = () => {
    ...
    return (
      <div>
        ...
        <PlainLabel
          disPlayText={GetClockDisplayFreqValue(Number(DPLL_CLOCK_FREQ.value))}
          ...
          redColorStatus={!isOutputFrequencyValid(DPLL_CLOCK_FREQ.value)}
        />
      </div>
    );
  };
    ...
    <div>
      ...
      <Dialog
        ...
        <FDPLLAutoCalculation
          componentId={componentId}
          expectedOutputFreq={DPLL_CLOCK_FREQ.value}
          close={() => {
            setDialogStatus(false);
          }}
