...
const CustomLogic = (props: { cx: (...classNames: string[]) => string }) => {
  ...
  const epll2freq = useStringSymbol({ componentId, symbolId: 'ETHCLK2' });
  const epll1freq = useStringSymbol({ componentId, symbolId: 'ETHCLK1' });
  const upllfreq = useStringSymbol({ componentId, symbolId: 'USBCLK' });
  ...
  useEffect(() => {
    ...
    const refreshReferenceClockDisplay = (
      ...
      for (let newIndex = 0; newIndex < nReferenceClockLenght; newIndex++) {
        ...
        } else if (roselValue === 'EPLL1') {
          refclk1InputFreq = Number(epll1freq.value);
        } else if (roselValue === 'EPLL2') {
          refclk1InputFreq = Number(epll2freq.value);
        } else if (roselValue === 'USBPLL') {
    ...
    if (
      ...
      if (nosc === 'SPLL') {
        ...
        noscFreq = Number(epll2freq.value);
      } else if (nosc === 'UPLL') {
        noscFreq = Number(upllfreq.value);
      }
  ...
  return (
    <>
      ...
      <PlainLabel
        disPlayText={GetClockDisplayFreqValue(Number(sysClockFreq.value))}
        className={props.cx('sysclk_res1')}
      />
    </>
  );
};
