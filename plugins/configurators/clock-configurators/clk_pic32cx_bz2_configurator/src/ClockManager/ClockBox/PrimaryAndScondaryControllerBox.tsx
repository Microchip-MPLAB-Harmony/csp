...
const PrimaryAndScondaryControllerBox = (props: {
  ...
  const soscenBz2 = useComboSymbol({ componentId, symbolId: 'OSCCON_SOSCEN_VALUE' });
  ...
  let socenSymbolID = 'CONFIG_LPOSCEN';
  ...
  return (
    <div>
      ...
      <ComboRadio
        ...
        classResolver={props.cx}
      />
      {productFamily.value !== '' && productFamily.value !== 'PIC32CX_BZ2' && (
        <ComboBoxDefault
          componentId={componentId}
      ...
      {productFamily.value !== '' && productFamily.value === 'PIC32CX_BZ2' && (
        <Dropdown
          value={soscenBz2.value}
          options={soscenBz2.options}
          className={props.cx('soscen')}
          onChange={(e) => {
            configSymbolApi.setValue(componentId, soscenBz2.symbolId, e.value);
          }}
        />
      )}
