...
const MainClockControllerBox = (props: {
  ...
  const cmd_div1 = useKeyValueSetSymbol({
    componentId,
    symbolId: 'CONF_MCLK_CLKDIV0'
  });
  const cmd_div2 = useKeyValueSetSymbol({
    componentId,
    symbolId: 'CONF_MCLK_CLKDIV1'
  });
  const cmd_div3 = useKeyValueSetSymbol({
    componentId,
    symbolId: 'CONF_MCLK_CLKDIV2'
  });
  ...
  return (
    <div>
      ...
      />
      <PlainLabel
        disPlayText={'' + cmd_div2.selectedOption.replace('DIV', '')}
        className={props.cx('mclk1CPUDIVLabel')}
      />
      <PlainLabel
        disPlayText={'' + cmd_div3.selectedOption.replace('DIV', '')}
        className={props.cx('mclk2CPUDIVLabel')}
      />
      <SettingsDialog
