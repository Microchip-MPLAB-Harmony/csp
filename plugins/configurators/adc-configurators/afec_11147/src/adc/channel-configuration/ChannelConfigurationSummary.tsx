...
export default function ChannelConfigurationSummary() {
  const { channels } = useChannelConfiguration();
  ...
  const GainCell = (channel: Channel) => {
    return (
      <label style={{ whiteSpace: 'nowrap' }}>
        {channel.gain !== undefined ? channel.gain.value : 'loading...'}
      </label>
    );
  };

  const OffsetCell = (channel: Channel) => {
    return (
      <label style={{ whiteSpace: 'nowrap' }}>
        {channel.offset !== undefined ? channel.offset.value : 'loading...'}
      </label>
    );
  };
  ...
  const DualSampleAndHoldCell = (channel: Channel) => {
    if (channel.channelNumber > 5) {
      return <div></div>;
    }
    if (channel.dualSampleAndHold?.value) {
      return (
        <i
          className='pi pi-check'
          style={{ color: 'green', fontSize: '1.25rem' }}></i>
