...
<#macro CONFIGURE_GCLK INDEX PCLKEN GCLKEN GCLKCSS GCLKDIV>
	...
	PMC_REGS->PMC_PCR = <#if INDEX == 0> PMC_PCR_PID(69) <#else> PMC_PCR_PID(70)</#if> | PMC_PCR_CMD_Msk ${PCLKEN?then('| PMC_PCR_EN_Msk', '')}  ${GCLKEN?then('| PMC_PCR_GCLKEN_Msk | PMC_PCR_GCLKCSS_${GCLKCSS} | PMC_PCR_GCLKDIV(${GCLKDIV-1})', '')};
