...
<#if RTC_MODULE_SELECTION = "MODE0">
    ...
    RTC_REGS->MODE0.INTENCLR |= RTC_MODE0_INTENCLR_Msk;

    /* 32-bit Period Value */
    RTC_REGS->MODE0.COMP = RTC_MODE0_PERIOD;
<#else>
    /* Set Operating Mode, Prescaler and COUNT Read Synchronization */
    <@compress single_line=true>RTC_REGS->MODE1.CTRLA |= RTC_MODE1_CTRLA_MODE(1) |
                                                            RTC_MODE1_CTRLA_PRESCALER(${RTC_MODE1_PRESCALER}) |
                                                            RTC_MODE1_CTRLA_COUNTSYNC_Msk;</@compress>

    /* Disable all Interrupts */
    RTC_REGS->MODE1.INTENCLR |= RTC_MODE1_INTENCLR_Msk;

<#if RTC_MODE1_INTERRUPT = false>
    <#if RTC_MODE1_GENERATE_COMPARE0_API = true>
    /* 16-bit Timer Compare 0 Value */
    RTC_REGS->MODE1.COMP[0] = RTC_MODE1_COMPARE0_VALUE;

    </#if>
    <#if RTC_MODE1_GENERATE_COMPARE1_API = true>
    /* 16-bit Timer Compare 0 Value */
    RTC_REGS->MODE1.COMP[1] = RTC_MODE1_COMPARE1_VALUE;

    </#if>
</#if>
    /* 16-bit Period Value */
    RTC_REGS->MODE1.PER = RTC_MODE1_PERIOD;
...
void RTC${RTC_INDEX}_FrequencyCorrect (int8_t correction)
{
    ...
    RTC_REGS->${RTC_MODULE_SELECTION}.FREQCORR = newCorrectionValue;

    while((RTC_REGS->${RTC_MODULE_SELECTION}.SYNCBUSY & RTC_${RTC_MODULE_SELECTION}_SYNCBUSY_FREQCORR_Msk) == RTC_${RTC_MODULE_SELECTION}_SYNCBUSY_FREQCORR_Msk)
    {
...
<#if RTC_MODULE_SELECTION = "MODE0" >
    ...
    if( (RTC_REGS->MODE0.INTFLAG & RTC_MODE0_INTFLAG_${RTC_MODE0_PERIOD}_Msk ) == RTC_MODE0_INTFLAG_${RTC_MODE0_PERIOD}_Msk)
    {
        periodIntervalComplete = true;

        /* Clear Periodic Interval Interrupt */
        RTC_REGS->MODE0.INTFLAG = RTC_MODE0_INTFLAG_${RTC_MODE0_PERIOD}_Msk;
    }

<#elseif RTC_MODULE_SELECTION = "MODE1" >
    /* Check for the Period interval has completed */
    if( (RTC_REGS->MODE1.INTFLAG & RTC_MODE1_INTFLAG_${RTC_MODE1_PERIOD}_Msk ) == RTC_MODE1_INTFLAG_${RTC_MODE1_PERIOD}_Msk)
    {
        periodIntervalComplete = true;

        /* Clear Periodic Interval Interrupt */
        RTC_REGS->MODE1.INTFLAG = RTC_MODE1_INTFLAG_${RTC_MODE1_PERIOD}_Msk;
    }
</#if>
...
bool RTC${RTC_INDEX}_Timer32PeriodHasExpired ( void )
{
    ...
    if((RTC_REGS->MODE0.INTFLAG & RTC_MODE0_INTFLAG_CMP0_Msk) == RTC_MODE0_INTFLAG_CMP0_Msk)
    {
        periodHasExpired = true;

        /* Clear Period Interrupt */
        RTC_REGS->MODE0.INTFLAG = RTC_MODE0_INTFLAG_CMP0_Msk;
    }
...
bool RTC${RTC_INDEX}_Timer32CounterHasOverflowed ( void )
{
    ...
    if((RTC_REGS->MODE0.INTFLAG & RTC_MODE0_INTFLAG_OVF_Msk) == RTC_MODE0_INTFLAG_OVF_Msk)
    {
        counterHasOverflowed = true;

        /* Clear Counter Overflow Interrupt */
        RTC_REGS->MODE0.INTFLAG = RTC_MODE0_INTFLAG_OVF_Msk;
    }
...
bool RTC${RTC_INDEX}_Timer16PeriodHasExpired ( void )
{
    ...
    if((RTC_REGS->MODE1.INTFLAG & RTC_MODE1_INTFLAG_OVF_Msk ) == RTC_MODE1_INTFLAG_OVF_Msk)
    {
        periodHasExpired = true;

        /* Clear Period/Overflow Interrupts */
        RTC_REGS->MODE1.INTFLAG = RTC_MODE1_INTFLAG_OVF_Msk;
    }
...
bool RTC${RTC_INDEX}_Timer16Compare0HasMatched(void)
{
    ...
    if((RTC_REGS->MODE1.INTFLAG & RTC_MODE1_INTFLAG_CMP0_Msk ) == RTC_MODE1_INTFLAG_CMP0_Msk)
    {
        compare0ValueMatch = true;

        /* Clear Compare 0 Match Flag */
        RTC_REGS->MODE1.INTFLAG = RTC_MODE1_INTFLAG_CMP0_Msk;
    }
...
bool RTC${RTC_INDEX}_Timer16Compare1HasMatched(void)
{
    ...
    if((RTC_REGS->MODE1.INTFLAG & RTC_MODE1_INTFLAG_CMP1_Msk ) == RTC_MODE1_INTFLAG_CMP1_Msk)
    {
        compare1ValueMatch = true;

        /* Clear Compare 1 Match Flag */
        RTC_REGS->MODE1.INTFLAG = RTC_MODE1_INTFLAG_CMP1_Msk;
    }
...
void RTC${RTC_INDEX}_Timer32Start ( void )
{
    ...
    RTC_REGS->MODE0.INTFLAG = RTC_MODE0_INTFLAG_Msk;

<#if RTC_MODE0_INTERRUPT = true>
<#if RTC_MODE0_PERIOD != "NO">
    /* Enable Period and Overflow Interrupt */
    RTC_REGS->MODE0.INTENSET |= RTC_MODE0_INTENSET_${RTC_MODE0_PERIOD}_Msk | RTC_MODE0_INTENSET_OVF_Msk;
<#else>
    /* Enable Overflow Interrupt */
    RTC_REGS->MODE0.INTENSET |= RTC_MODE0_INTENSET_OVF_Msk;
</#if>
</#if>

    /* Enable RTC */
    RTC_REGS->MODE0.CTRLA |= RTC_MODE0_CTRLA_ENABLE_Msk;

    while((RTC_REGS->MODE0.SYNCBUSY & RTC_MODE0_SYNCBUSY_ENABLE_Msk) == RTC_MODE0_SYNCBUSY_ENABLE_Msk)
    {
...
void RTC${RTC_INDEX}_Timer32CounterSet ( uint32_t count )
{
    ...
    RTC_REGS->MODE0.COUNT = count;

    while((RTC_REGS->MODE0.SYNCBUSY & RTC_MODE0_SYNCBUSY_COUNT_Msk) == RTC_MODE0_SYNCBUSY_COUNT_Msk)
    {
...
void RTC${RTC_INDEX}_Timer32PeriodSet ( uint32_t period )
{
    ...
    RTC_REGS->MODE0.CTRLA |= RTC_MODE0_CTRLA_MATCHCLR_Msk;

<#if RTC_MODE0_INTERRUPT = true>
    /* Enable Compare Interrupt */
    RTC_REGS->MODE0.INTENSET |= RTC_MODE0_INTENSET_CMP0_Msk;

</#if>
    /* Set 32Bit Compare Value */
    RTC_REGS->MODE0.COMP = period;

    while((RTC_REGS->MODE0.SYNCBUSY & RTC_MODE0_SYNCBUSY_COMP0_Msk) == RTC_MODE0_SYNCBUSY_COMP0_Msk)
    {
...
uint32_t RTC${RTC_INDEX}_Timer32CounterGet ( void )
{
    ...
    while((RTC_REGS->MODE0.SYNCBUSY & RTC_MODE0_SYNCBUSY_COUNTSYNC_Msk) == RTC_MODE0_SYNCBUSY_COUNTSYNC_Msk)
    {
        /* Wait for Synchronization before reading value from Count Register */
    }

    /* Get 32Bit Count Value from COUNT register */
    count = RTC_REGS->MODE0.COUNT;
...
{
    /* Clear all Interrupts */
    RTC_REGS->MODE1.INTFLAG = RTC_MODE1_INTFLAG_Msk;

<#if RTC_MODE1_INTERRUPT = true>
<#if RTC_MODE1_PERIOD != "NO">
    /* Enable Period and Overflow Interrupt */
    RTC_REGS->MODE1.INTENSET |= RTC_MODE1_INTENSET_${RTC_MODE1_PERIOD}_Msk | RTC_MODE1_INTENSET_OVF_Msk;
<#else>
    /* Enable Overflow Interrupt */
    RTC_REGS->MODE1.INTENSET |= RTC_MODE1_INTENSET_OVF_Msk;
</#if>
</#if>

    /* Enable RTC */
    RTC_REGS->MODE1.CTRLA |= RTC_MODE1_CTRLA_ENABLE_Msk;

    while((RTC_REGS->MODE1.SYNCBUSY & RTC_MODE1_SYNCBUSY_ENABLE_Msk) == RTC_MODE1_SYNCBUSY_ENABLE_Msk)
    {
        /* Wait for Synchronization after Enabling RTC */
    }
}
...
void RTC${RTC_INDEX}_Timer16CounterSet ( uint16_t count )
{
    ...
    RTC_REGS->MODE1.COUNT = count;

    while((RTC_REGS->MODE1.SYNCBUSY & RTC_MODE1_SYNCBUSY_COUNT_Msk) == RTC_MODE1_SYNCBUSY_COUNT_Msk)
    {
        /* Wait for Synchronization after writing value to Count Register */
    }
}
...
void RTC${RTC_INDEX}_Timer16PeriodSet ( uint16_t period )
{
    ...
    RTC_REGS->MODE1.PER = period;

    while((RTC_REGS->MODE1.SYNCBUSY & RTC_MODE1_SYNCBUSY_PER_Msk) == RTC_MODE1_SYNCBUSY_PER_Msk)
    {
...
uint16_t RTC${RTC_INDEX}_Timer16CounterGet ( void )
{
    ...
    while((RTC_REGS->MODE1.SYNCBUSY & RTC_MODE1_SYNCBUSY_COUNTSYNC_Msk) == RTC_MODE1_SYNCBUSY_COUNTSYNC_Msk)
    {
        /* Wait for Synchronization after reading value from Count Register */
    }

    /* Get 16Bit Count Value from COUNT Register */
    count = RTC_REGS->MODE1.COUNT;
...
void RTC${RTC_INDEX}_Timer16Compare0Set ( uint16_t comparisionValue )
{
     ...
     RTC_REGS->MODE1.COMP[0] = comparisionValue;

     while((RTC_REGS->MODE1.SYNCBUSY & RTC_MODE1_SYNCBUSY_COMP0_Msk) == RTC_MODE1_SYNCBUSY_COMP0_Msk)
     {
         /* Wait for Synchronization after writing Compare Value */
     }
}
...
{
    /* Set 16Bit Compare Value */
    RTC_REGS->MODE1.COMP[1] = comparisionValue;

    while((RTC_REGS->MODE1.SYNCBUSY & RTC_MODE1_SYNCBUSY_COMP1_Msk) == RTC_MODE1_SYNCBUSY_COMP1_Msk)
    {
        /* Wait for Synchronization after writing Compare Value */
    }
}
...
void RTC${RTC_INDEX}_Timer16Compare1EventEnable (bool enable)
{
    if(enable)
    {
        ...
        RTC_REGS->MODE1.INTENSET |= RTC_MODE1_INTENSET_CMP1_Msk;
    }
    else
    {
...
void RTC${RTC_INDEX}_Timer16Compare0EventEnable(bool enable)
{
    if(enable)
    {
        ...
        RTC_REGS->MODE1.INTENSET |= RTC_MODE1_INTENSET_CMP0_Msk;
    }
    else
    {
...
<#if RTC_MODE0_PERIOD != "NO">
    ...
    if((RTC_REGS->MODE0.INTFLAG & RTC_REGS->MODE0.INTENSET) & RTC_32BIT_INTERRUPT_MASK)
    {
        /* Update the event in RTC Peripheral Callback object */
        rtc${RTC_INDEX}Obj.timer32BitEvent = RTC_REGS->MODE0.INTFLAG;
...
<#if RTC_MODE1_PERIOD != "NO">
    ...
    if((RTC_REGS->MODE1.INTFLAG & RTC_REGS->MODE1.INTENSET) & RTC_16BIT_INTERRUPT_MASK)
    {
        /* Update the event in RTC Peripheral Callback object */
        rtc${RTC_INDEX}Obj.timer16BitEvent = RTC_REGS->MODE1.INTFLAG;
