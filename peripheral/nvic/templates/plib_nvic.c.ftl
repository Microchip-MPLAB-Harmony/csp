...
<#list NVIC_VECTOR_MIN..NVIC_VECTOR_MAX as i>
    <#list 0..NVIC_VECTOR_MAX_MULTIPLE_HANDLERS as j>
        <#assign NVIC_VECTOR_GENERIC_NAME = "NVIC_" + i + "_" + j + "_GENERIC_NAME">
    ...
    NVIC_SetPriority(${.vars[NVIC_VECTOR_GENERIC_NAME]}_IRQn, ${.vars[NVIC_VECTOR_PRIORITY]});
                    </#if>
                    <#if .vars[NVIC_VECTOR_ENABLE_GENERATE]?has_content && (.vars[NVIC_VECTOR_ENABLE_GENERATE] != false)>
    NVIC_EnableIRQ(${.vars[NVIC_VECTOR_GENERIC_NAME]}_IRQn);
                    </#if>
                    <#break>
                </#if>
            </#if>
