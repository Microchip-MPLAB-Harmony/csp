...
#pragma weak ${.vars[NVIC_VECTOR_GENERIC_HANDLER]}=Dummy_Handler
    ...
    <#else>
        ...
        <#if (BARE_METAL && [-5,-2]?seq_contains(i)) || ((i == -1) && (USE_SYSTICK_INTERRUPT == false)) || (i == -1 && (.vars[NVIC_VECTOR_ENABLE] == false)) || ((.vars[NVIC_VECTOR_NONSECURE])?? && (.vars[NVIC_VECTOR_NONSECURE] == "NON-SECURE") && (.vars[NVIC_VECTOR_ENABLE] == false))>
...
#pragma weak ${.vars[NVIC_VECTOR_GENERIC_HANDLER]}=Dummy_Handler
    ...
    <#else>
        ...
        <#assign NVIC_VECTOR_ENABLE = "NVIC_" + i + "_" + "0_ENABLE">
        <#assign NVIC_VECTOR_HANDLER = "NVIC_" + i + "_" + "0_HANDLER">
        <#if (BARE_METAL && [-5,-2]?seq_contains(i)) || (.vars[NVIC_VECTOR]?has_content && (.vars[NVIC_VECTOR] != "None") && (.vars[NVIC_VECTOR_ENABLE] == false))>
            <#assign handler = .vars[NVIC_VECTOR_HANDLER]>
            <#if !dummyHandlers?seq_contains(handler)>
                <#assign dummyHandlers = dummyHandlers + [handler] />
                <#if COMPILER_CHOICE == "XC32">
